...
- Name: pipeline.spec.ts >> Pipeline >> shows agent activities when pipeline is active
- Location: tests\e2e\pipeline.spec.ts:32:7
...
Locator: getByText('Agent Activities').or(locator('[class*=\'agent\']').first()).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('Agent Activities').or(locator('[class*=\'agent\']').first()).first()
