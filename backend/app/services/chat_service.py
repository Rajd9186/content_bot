...
class ResearchCopilotService:
    ...
    async def get_context(self, project_id: uuid.UUID) -> str:
        project = await self.project_repo.get(project_id)
        content = await self.content_repo.get_latest_by_project(project_id)
    ...
    async def chat(self, project_id: uuid.UUID, message: str, history: List[ChatMessage]) -> ChatResponse:
        ...
        if "regenerate" in msg_lower or "re-run" in msg_lower:
            ...
            tool_calls.append(ChatToolCall(tool="trigger_workflow", parameters={"project_id": str(project_id), "mode": "v2"}))

        elif "contradiction" in msg_lower or "conflict" in msg_lower:
            conts = await self.contradiction_repo.get_by_project(project_id)
            if conts:
            ...
            sources = await self.source_repo.get_by_project(project_id)
            ...
            wf = await self.workflow_repo.get_latest_by_project(project_id)
            if wf:
            ...
            response_text = (
                f"I'm your Research Copilot for the '{project_id}' project. "
                "I can help you investigate evidence, find contradictions, or control the research agents. "
                "What would you like to know about the current findings?"
            )
