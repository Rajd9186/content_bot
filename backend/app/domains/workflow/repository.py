...
class WorkflowRepository(BaseRepository[WorkflowJob]):
    ...
    async def heartbeat(self, job_id: str, expected_version: int) -> bool:
        """Update updated_at for an active job (heartbeat).

        Returns True if the heartbeat was applied, False on version conflict.
        """
        stmt = (
            update(WorkflowJob)
            .where(and_(
