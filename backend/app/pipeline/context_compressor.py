...
class ContextCompressor:
    ...
    def _select_context(
        ...
        if agent_type == "writer":
            ...
            truncated_key_points = [
                self._summarize_or_truncate_text(str(kp), 100) # Limit each key point
                for kp in research.get("key_points", [])[:8]
            ]
            truncated_plan_sections = [
                self._summarize_or_truncate_text(str(sec), 150) # Limit each plan section
                for sec in plan.get("sections", [])[:8]
            ]
            truncated_outline_sections = [
                self._summarize_or_truncate_text(str(sec), 150) # Limit each outline section
                for sec in outline.get("sections", [])[:8]
            ]
            return {
                **core,
                "research_summary": self._summarize_or_truncate_text(
                    research.get("summary", ""), 1000, # More aggressive summary truncation
                ),
                "key_points": truncated_key_points,
                "statistics": research.get("statistics", [])[:3], # Reduce number of statistics
    ...
    def _truncate_context(
        ...
        # For writer agent, we can have more specific truncation
        if agent_type == \"writer\":
    ...
    def _truncate_writer_context(
        ...
        # Prioritize core info, then plan/outline, then research summary, then key points
        truncated: dict[str, Any] = {}
        remaining_budget = char_budget

        # Core info - try to keep full
        for key in [\"topic\", \"audience\", \"tone\", \"goals\"]:
            if key in context and isinstance(context[key], str):
                val = context[key]
                truncated[key] = val
                remaining_budget -= len(val) + len(key) + 2 # key: val\\n

        # Plan and Outline sections - important for structure
        ...
        section_char_budget = int(remaining_budget * 0.4) # Allocate 40% for sections
        truncated_sections = []
        for section in combined_sections:\
            section_str = str(section)\
            if len(section_str) <= section_char_budget // len(combined_sections) if combined_sections else 1:\
                truncated_sections.append(section)\
                remaining_budget -= len(section_str)\
            else:\
                truncated_sections.append(self._summarize_or_truncate_text(section_str, section_char_budget // len(combined_sections) if combined_sections else 1))\
                remaining_budget -= (section_char_budget // len(combined_sections) if combined_sections else 1)
        truncated[\"plan_outline_sections\"] = truncated_sections


        # Research Summary - often large, can be aggressively truncated
        research_summary = context.get(\"research_summary\", \"\")
        summary_char_budget = int(remaining_budget * 0.3) # Allocate 30% for summary
        ...
        # Key Points - also can be truncated
        key_points = context.get(\"key_points\", [])
        key_point_char_budget = int(remaining_budget * 0.3) # Allocate 30% for key points
        truncated_key_points = []
        for kp in key_points:
            kp_str = str(kp)
            truncated_key_points.append(self._summarize_or_truncate_text(
                kp_str, max(50, key_point_char_budget // len(key_points) if key_points else 1),
            ))
        truncated[\"key_points\"] = truncated_key_points

        # Any other fields, add with general truncation if space permits
        for key, value in context.items():
            if key not in truncated:
                val_str = str(value)
                if remaining_budget > 100: # Arbitrary threshold for other items
                    max_len = max(50, int(len(val_str) * (remaining_budget / (len(val_str) + 1))))
                    truncated[key] = self._summarize_or_truncate_text(val_str, max_len)
                    remaining_budget -= len(truncated[key])
                else:
